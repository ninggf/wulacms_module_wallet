...
class Wallet {
    ...
    public function pay(Currency $currency, string $withdrawId, string $uid, string $channel, string $account): bool {
        try {
            ...
            $withdrawTable = $this->realtb('wallet_withdraw_order', $currency['id']);
            $withdraw      = $this->walletdb->queryOne('SELECT * FROM ' . $withdrawTable . ' WHERE id = %s AND status = %s', $withdrawId, 'A');
            if (!$withdraw) {
    ...
    public function exchange(Currency $currencyForm, Currency $currencyTo, string $amount, float $discount = 1.0): ?string {
        ...
        try {
            ...
            // 实际转出币种金额(最大单位)
            $total = bcdiv(bcmul($amount, $discount), 10000, $currencyForm->scale);
            //创建兑换记录
            ...
            $exchange['total']         = $currencyForm->toUint($total);// 实际转出币种金额(最大单位)
